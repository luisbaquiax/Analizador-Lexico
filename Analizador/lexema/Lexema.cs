    ...
    public class Lexema
    {
        ...
        public void mostrarLexemas()
        {
            ...
            for (int i = 0; i < palabras.Length; i++)
            {
                ...
                if (verSiEsMonedaGuatemala(palabras[i]))
                {
                    insertarPalabraMoneda(palabras[i]);
                }
            }
        ...
        {
            String moneda = "Moneda de Guatemala: \n";
            for (int i = 0; i < this.monedaGuatemala.Length; i++)
            {
                if (monedaGuatemala[i] != null)
                {
                    moneda += monedaGuatemala[i] + "\n";
                }
            }
            return moneda;
        }

        public Boolean verSiEsMonedaGuatemala(String palabra)
        {

            char[] subcadena = palabra.ToCharArray();

            String caracter = "";

            for (int i = 1; i < subcadena.Length; i++)
            {
                caracter += subcadena[i];
            }
            for (int i = 0; i < subcadena.Length; i++)
            {
                if (subcadena[0] == 'Q' && (esNumero(caracter)))
                {
                    return true;
                }
            }
        ...
        public void insertarPalabraMoneda(String moneda)
        {
            for (int i = 0; i < this.monedaGuatemala.Length; i++)
            {
                if (this.monedaGuatemala[i] == null)
                {
                    this.monedaGuatemala[i] = moneda;
                    return;
                }
            }
        }
